...
class Block:
    ...
    def hash_block(self):
        ...
        record = str(self.record.sender).encode()
        sha.update(record)

        record = str(self.record.receiver).encode()
        sha.update(record)

        record = str(self.record.amount).encode()
        sha.update(record)
...
def setup():
    ...
    new_record = Record("Genesis", "First Block", 0.00)
    return PyChain([Block(new_record, 0.00, np.random.randint(1, 100))])
...
tx_sender = st.text_input("Sender Address:")

# @TODO:
# Add an input area where you can get a value for `receiver` from the user.
tx_receiver = st.text_input("Receiver Address:")

# @TODO:
# Add an input area where you can get a value for `amount` from the user.
tx_amount = float(st.text_input("Amount to send:", "0.00"))
...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_record = Record(tx_sender, tx_receiver, tx_amount)
    new_block = Block(
        record=new_record,
        creator_id=np.random.randint(1,100),
